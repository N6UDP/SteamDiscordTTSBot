...
- **2026-06-26**: Fixed Steam login broken by Valve deprecating legacy username/password `LogOn` for non-web clients. `Steam.cs` now uses the modern token flow: `steamClient.Authentication.BeginAuthSessionViaCredentialsAsync` → `PollingWaitForResultAsync()` → `LogOn` with the returned `RefreshToken`. Steam Guard codes are sourced from Discord via `SteamGuardBridge.cs` (`DiscordAuthenticator : IAuthenticator`): on first login the bot posts a prompt to the channel in `SteamGuard_DiscordChannelId` and the operator replies with the emailed code (also logged to console as fallback). Tokens + guard data are cached in `steamauth.json` (atomic write + timestamped `.bak`, gitignored via `*.json`) so restarts skip the prompt; an expired stored token is detected in `OnLoggedOn` (`usingStoredToken`), cleared (keeping guard data), and falls back to full credential auth. `OnConnected` is `async void` and uses a per-attempt generation guard (`var thisClient = steamClient` + `ReferenceEquals` re-checks) so a stale auth flow from a dropped connection can't tear down or mis-`LogOn` a newer connection. New config key `SteamGuard_DiscordChannelId` (App.config + App.config.example). `InvalidPassword` (credential path) and `AccountLogonDenied` are permanent non-retryable failures.
