...
**NEVER read, output, or reference `App.config` directly** — it contains secrets (bot token, Steam credentials, HuggingFace token). It is listed in `.copilotignore`. Use `App.config.example` as the reference for config structure. When config changes are needed, describe them in terms of `App.config.example` keys.
